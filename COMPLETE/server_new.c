...
#define SIZE 20
#define NUMBER_OF_MINE (SIZE*SIZE/4)
...
char* get_inspiration(void)
{
    ...
    int byte_num;
    for(;;){
        system("fortune > fortune.txt");
        if((fortunefd = fopen("fortune.txt", "r")) == NULL)
            continue;
        byte_num = 0;
        while(getc(fortunefd) != EOF){
            byte_num++;
        }
        rewind(fortunefd);
        if(byte_num < 90){
            printf("%d\n", byte_num);
            fortune = (char *)malloc(sizeof(char)*byte_num);
            fread(fortune, sizeof(char), byte_num-1, fortunefd);
            fortune[byte_num-1] = '\0';
            fclose(fortunefd);
            return fortune;
            break;
        }else{
...
void Run_server(void)
{
    ...
    struct utsname uts;
    char loser[9] = "You out!";
    char *inspiration;
    int inspiration_size;
                        ...
                        if ((nbytes = recv(i, buf, sizeof buf, 0)) <= 0) {
                            ...
                            if(!strcmp(buf, "refresh")){
                                ...
                                sendall(i, pkg, &sendsize);
                                    ...
                                    sendall(i, user_num_string, &user_num_string_size);
                            }else if(!strcmp(buf, "mines")){
                                    sprintf(mines_num_string, "%d", NUMBER_OF_MINE);
                                    mines_num_string_size = sizeof(mines_num_string);
                                    sendall(i, mines_num_string, &mines_num_string_size);
                            }else if(!strcmp(buf, "inspire")){
                                    inspiration = get_inspiration();
                                    inspiration_size = strlen(inspiration);
                                    sendall(i, inspiration, &inspiration_size);
                                    free(inspiration);
                                ...
                                if(check_for_mine(x,y)){
                                    creat_sad();
                                    shutdown(i, SHUT_RD); // connection closed or error occured
                                    send(i, loser, sizeof(loser), 0);
                                    user_num--;
                                    FD_CLR(i, &master); // remove from read set
